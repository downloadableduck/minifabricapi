...
import net.fabricmc.fabric.mixin.client.rendering.ModelMixin;
 ...
 */
public final class TransformCopyingModel<S, D> extends Model<@NotNull Pair<S, D>> implements ModelExtensions {
	...
	public void setupAnim(Pair<S, D> state) {
        ...
        ((FabricModel<Object>) delegate).copyTransforms(source);
	...
	public @Nullable ModelPart getChildPart(String name) {
		return ((FabricModel<Object>) delegate).getChildPart(name);
	}

	@Override
	public void copyTransforms(Model model) {
        ((FabricModel<Object>) delegate).copyTransforms(model);
	}
}
